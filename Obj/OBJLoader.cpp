...
/*		currentFace->theNormals[0] =  m_vFaces[i].m_uiNormalIdx[0];
	...
	{	
		...
		strcpy(temp.matName,current.matName);
		strcpy(temp.fileName, current.textureName);
...

	{
		...
		else if(identifierStr == "newmtl")
		{
			...
			if(foundNewMaterial)
			{
				...
				newMaterial.glIndex = TextureHandler::lookUpTexture(buf, true);
			...
			sscanf(line, "%s", newMaterial.matName);
...

		{
			...
			sscanf(line, "llum %i", &newMaterial.illum);
...

		{
			...
			sscanf(line, "%f %f %f", &newMaterial.ambientCol[0], &newMaterial.ambientCol[1], &newMaterial.ambientCol[2]);
...

		{
			...
			sscanf(line, "Kd %f %f %f", &newMaterial.diffuseCol[0], &newMaterial.diffuseCol[1], &newMaterial.diffuseCol[2]);
...

		{
			...
			sscanf(line, "Ks %f %f %f", &newMaterial.specCol[0], &newMaterial.specCol[1], &newMaterial.specCol[2]);
...

		{
			...
			sscanf(line, "%s", &newMaterial.textureName); 
...
void OBJLoader::getDirectory(const char *source, char * result)
{
	char lastPos = 0;
	int length = (int) strlen(source);
	for (int count = 0; count < length; count++)
	{
		if (source[count] == '/') lastPos = count;
	}

	if (lastPos == 0) sprintf(result, ".");
	else
	{
		memcpy(result, source, sizeof(char) * lastPos);
		result[lastPos] ='\0'; //put back on null termination
	}
}
