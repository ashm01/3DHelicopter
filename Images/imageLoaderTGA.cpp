...
tImageTGA* ImageLoader::LoadTGA(const char *filename)
{
	...
	if(imageType != TGA_RLE)
	{
		...
		{
			channels = bits / 8;
			pImageData->data = new unsigned char[width * height];
			
			// Load in all the pixel data line by line
			for(int y = 0; y < height; y++)
			{
				// Store a pointer to the current line of pixels
				unsigned char *pLine = &(pImageData->data[width * y]);
				
				// Read in the current line of pixels
				fread(pLine, width, 1, pFile);
			}
		}
...
int ImageLoader::createTGA(const char* strFileName, bool mipmap)
{
	...
	try
	{
		pImage = LoadTGA(strFileName);			// Load the image and store the data
	}
	catch (...)
	{
		//nothing to do so rethrow
		return 0;
	}
	...
	{
		glTexImage2D(GL_TEXTURE_2D, 0,  1, pImage->sizeX, 
			pImage->sizeY,  0, GL_RED, GL_UNSIGNED_BYTE, pImage->data);
	...
	else
	{
		...
		{
			glTexImage2D(GL_TEXTURE_2D, 0,  GL_RGBA16, pImage->sizeX, 
				pImage->sizeY,  0, textureType, GL_UNSIGNED_BYTE, pImage->data);
