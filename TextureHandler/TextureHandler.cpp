...
int TextureHandler::loadTexture(char *fileName, bool buildMipMaps)
{
	int textureID;

	try
	{
		char * resultTGA = strstr(fileName,".tga");
		char * resultDDS = strstr(fileName,".dds");
		char * resultPNG = strstr(fileName,".png");
		char * resultJPG = strstr(fileName,".jpg");

		if (resultTGA != NULL)
		{
			textureID = ImageLoader::createTGA(fileName,buildMipMaps);
		}
		else if(resultDDS != NULL)
		{
			textureID = ImageLoader::loadDDS(fileName);
		}
		else if(resultPNG != NULL)
		{
			textureID = ImageLoader::loadPNG(fileName);
		}
		else if(resultJPG != NULL)
		{
			textureID = ImageLoader::createJPG(fileName);
		}
		else
		{
			std::cout << " unsupported texture format for " << fileName << std::endl;
			std::cout << " loader supports: tga, dds, jpg, png " << std::endl;
		}
...
bool TextureHandler::searchMap(char *fileName, int &result)
{
...
int TextureHandler::lookUpTexture(char *fileName, bool buildMipMaps)
{
	int textureID;

	int tempRes;
	if (searchMap(fileName, tempRes)) 
	{
		textureID = tempRes;
	}
	else
	{
		textureID = loadTexture(fileName, buildMipMaps);
	}
